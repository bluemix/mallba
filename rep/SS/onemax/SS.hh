...
//Solution ----------------------------------------------------------------------------
  ...
  requires class Solution
  {
	public:
		...
		void to_Solution(char *_string_);
		unsigned int size() const;

		void initialize();
		double fitness ();

		void initialize(const Rarray<Solution*>& sols, unsigned size);
		void improve();
		double distance(const Solution &sol) const;
...
// Crossover  ----------------------------------------------------------------------------------
  ...
  requires class Crossover: public Intra_Operator
  {
	...
	public:
		...
		void cross(const Rarray<Solution*>& sols, Solution& sol) const;
...
// SetUpParams -------------------------------------------------------------------------------
  ...
  provides class SetUpParams
  {
	private:
		...
		unsigned int    _population_size1;		// number of individuals
		unsigned int    _population_size2;		// number of individuals
		unsigned int    _initial_population_size;		// number of individuals
	...
	public:
		...
		const unsigned int  population_size1() const;
		const unsigned int  population_size2() const;
		const unsigned int  initial_population_size() const;
		...
		void population_size1(const unsigned int val);
		void population_size2(const unsigned int val);
		void initial_population_size(const unsigned int val);
...
// Population ---------------------------------------------------------------------------------
  ...
  provides class Population
  {
	...
	public:
		...
		void select_div_parents(Rarray<Solution*>&p, Rarray<Solution*>&o);
...
// Selection ( Makes a random selection ) -----------------------------------------
  ...
  provides class Selection
  {
	...
	public:
		...
		virtual void setup(unsigned par1, unsigned par2) {};
  };
...
// Selection_Worst  ---------------------------------------------------------------------------------
  ...
  provides class Selection_SS: public Selection
  {
	public:
		Selection_SS(const Direction dir);
		virtual ~Selection_SS();

		friend ostream& operator<< (ostream& os, const Selection_Tournament& sel);

	    virtual void prepare(Rarray<struct individual>& fitness_values,const bool remplace); // const;
		virtual struct individual select_one(const Rarray<Solution*>& to_select_1,const Rarray<Solution*>& to_select_2,const Rarray<struct individual>& fitness_values,const unsigned int order,const bool remplace) const;	
		virtual void setup(unsigned par1, unsigned par2);
		
	private:
		unsigned _rs1, _rs2;
		mutable Rarray<int> _order;
		mutable bool prepareb;

		int prepare_one(const Rarray<Solution*>& to_select_1,const Rarray<Solution*>& to_select_2,const Rarray<struct individual>& fitness_values, unsigned s) const;

		void prepare2(const Rarray<Solution*>& to_select_1,const Rarray<Solution*>& to_select_2,const Rarray<struct individual>& fitness_values,const bool remplace) const;
  };
