...
// SetUpParams -----------------------------------------------------------
 	...
 	istream& operator>> (istream& is, SetUpParams& setup)
	{
		...
		while (is.getline(buffer,MAX_BUFFER,'\n'))
		{
			...
			if (!(strcmp(command,"Inter-Operators"))) nb_section=1;
			if (!(strcmp(command,"LAN-configuration"))) nb_section=2;
				...
				case 0: switch (nb_param)
					{
						...
						case 2: setup.population_size1(op); break;
						case 3: setup.population_size2(op); break;
						case 4: setup.initial_population_size(op); break;
						case 5: setup.population_additional_size(op); break;
						case 6: setup.display_state(op); break;
					}
					nb_param++;
					break;
				case 1: setup._inter_operators.append(new unsigned int(op));
					setup.pool().inter_operator(op).setup(buffer);
					break;
				case 2: if (nb_LAN_param>=3) break;
					switch (nb_LAN_param)
					{
		...
		setup.pool().intra_operators().append(Intra_Operator::create(0));
		sprintf(buffer,"%d %d", setup.population_size1(), setup.population_size2());
		setup.pool().intra_operator(nb_io).setup(buffer);
		setup.pool().selector(6).setup(setup.population_size1(), setup.population_size2());
	...
	ostream& operator<< (ostream& os, const SetUpParams& setup)
	{
		...
		os << "\t" << "Independent runs : " << setup.independent_runs()    << endl
		   ...
		   << "\t" << "Size of Population1: " <<  setup.population_size1() << endl
		   << "\t" << "Size of Population2: " <<  setup.population_size2() << endl
		   << "\t" << "Initial Size of Population: " <<  setup.initial_population_size() << endl
	...
	const unsigned int SetUpParams::population_size1() const
	{
		return _population_size1;
	}

	const unsigned int SetUpParams::population_size2() const
	{
		return _population_size2;
	}

	const unsigned int SetUpParams::initial_population_size() const
	{
		return _initial_population_size;
	}
	...
	void SetUpParams::population_size1(const unsigned int val)
	{
		_population_size1=val;
	}

	void SetUpParams::population_size2(const unsigned int val)
	{
		_population_size2=val;
	}

	void SetUpParams::initial_population_size(const unsigned int val)
	{
		_initial_population_size=val;
	}
...
// Population ------------------------------------------------------
	...
	:_parents(setup.population_size1() + setup.population_size2()),
	 _fitness_values(setup.population_size1()+setup.population_size2()),
	 _new_parents(setup.population_size1()+setup.population_size2()),
	 _offsprings(setup.population_additional_size()),
	 _setup(setup),_pbm(pbm),
	 _evaluations(0)
	{
	...
	{
		unsigned k = 0, i = 0;
		Rarray<Solution*> aux_pop(_setup.initial_population_size());
		Rarray<Solution*> aux_pop2(aux_pop.size() - _parents.size());

		for(k=0; k < aux_pop.size(); ++k)
		{
			aux_pop[k] = new Solution(_pbm);
			aux_pop[k]->initialize(aux_pop,k);	
			aux_pop[k]->improve();
		}
		
		for (i=0, k = 0;i<_parents.size();i++,k++)
		{
			*_parents[i] = *aux_pop[k];
			_fitness_values[i].index = k;
			_fitness_values[i].change = true;
			delete aux_pop[k];
		}
		
		for (i=0;i<aux_pop2.size();i++,k++)
		{
			aux_pop2[i] = new Solution(_pbm);
			*aux_pop2[i] = *aux_pop[k];

			delete aux_pop[k];
		}
	
		select_div_parents(_parents, aux_pop2);
		evaluate_parents();	
		
		for (i=0;i<aux_pop2.size();i++)
			delete aux_pop2[i];
	}
	...
	void Population::evolution()
	{
		...
		for (unsigned i = 0; i < _offsprings.size(); i++)
			_offsprings[i]->improve();

		evaluate_offsprings();
//cout << "pop: "; 
//for(unsigned i = 0; i < _parents.size(); i++)  cout << _parents[i]->fitness() << " ";
//cout << endl << "off: ";
//for(unsigned i = 0; i < _offsprings.size(); i++)  cout << _offsprings[i]->fitness() << " ";
...
//cout << endl << endl;
	...
	{
		const int max = (_parents.size()>_offsprings.size())?_offsprings.size():_parents.size();
		for (int i=0;i<max;i++)
		{		
			*_offsprings[i] = *_parents[i];
		}
	}

	void Population::select_offsprings()
	{
		_setup.pool().selector(6).prepare(_fitness_aux,false);
		...
		for (int i=0;i<ps;i++)
		{
			aux[i] = _setup.pool().selector(6).select_one(_parents,_offsprings,_fitness_aux,i,false);
			if(aux[i].index < ps)
			{
		...
		if(!change)
		{
			...
			{
				aux_pop[k] = new Solution(_pbm);
				if(k < ps) *aux_pop[k] = *_parents[k];
				else aux_pop[k]->initialize(aux_pop,k);	
			}
	...
	void Population::select_div_parents(Rarray<Solution*>&p, Rarray<Solution*>&o)
	{

		int i=0;
		_fitness_aux=Rarray<struct individual>(p.size() + o.size());
	    for (i=0;i<p.size();i++)
		{
			Evaluate(p[_fitness_values[i].index],_fitness_values[i]);
			_fitness_aux[i] = _fitness_values[i];
		}

		for (int j=i;(j-i)<o.size();j++)
		{
			_fitness_aux[j].index=j;
			_fitness_aux[j].change=true;
			Evaluate(o[j-i],_fitness_aux[j]);
		}
		...
		for (int i=0;i<ps;i++)
		{
			aux[i] = _setup.pool().selector(6).select_one(p,o,_fitness_aux,i,false);
			if(aux[i].index < ps)
			{
				*_new_parents[i] = *p[aux[i].index];
				aux[i].index = i;					
			}
			else
			{
				*_new_parents[i] = *p[aux[i].index-ps];
				aux[i].index = i;	
			}
		}
...
// Inter_operator -------------------------------------------------------------------
	...
	void Inter_Operator::setup(char line[MAX_BUFFER])
	{
		...
		sscanf(line," %d %d %d %d %d ",&op,&new_migration_rate,&new_migration_size,&new_migration_selection_1,&new_migration_selection_conf_1);
	...
	void Migration::execute(Population& pop,const unsigned long current_generation,NetStream& _netstream,const bool synchronized,const unsigned int check_asynchronous) const
	{
		  ...
		  && (current_generation!=pop.setup().nb_evolution_steps())) // in this generation this operator have to be applied
		{
			...
			if (synchronized)    // synchronous mode: blocked until data are received	
			{
				...
				_netstream << pack_begin; 
				pop.setup().pool().selector(6).prepare(pop.fitness_values(),true);
			 	for (int i=0;i<migration_size;i++)
				{
	                // select individual to be remplaced
					struct individual ind;
					solution_received=new Solution(pop.parents()[0]->pbm());
					_netstream >> *solution_received;
					
					Rarray<Solution*> o(1);
					o[0] = solution_received;
				   	
					ind = pop.setup().pool().selector(6).select_one(
					pop.parents(),pop.offsprings(),pop.fitness_values(),0,true);

					// remplace policy
					if ((ind.index >= 0) && (ind.index < pop.parents().size()))
					{
						for(int j = 0; j < pop.parents().size(); j++)
						{
							if(pop.fitness_values()[j].index == ind.index)
							{
								pop.fitness_values()[j].change = false;
								*pop.parents()[ind.index] = *solution_received;
								pop.fitness_values()[j].fitness = solution_received->fitness();
							}
						}
					}
...
// Selection ------------------------------------------------------------
	...
	ostream& operator<< (ostream& os, const Selection& sel)
	{
		switch (sel.number_selection())
		{
			...
			case 6: os << (Selection_SS&)sel; break;
		}
		return os;
	}
...
// Selection_SS  ---------------------------------------------------------------------------------
	...
	Selection_SS::Selection_SS(const Direction dir):Selection(6,dir),_order(1),_rs1(0),_rs2(0),prepareb(false)
	{}
	
	Selection_SS::~Selection_SS()
	{}

	ostream& operator<< (ostream& os, const Selection_SS& sel)
	{
		os << "Scatter Search Replacement.";
		return os;
	}
		
	void Selection_SS::prepare(Rarray<struct individual>& fitness_values,const bool remplace)
	{
		prepareb = false;
	}
	
	struct individual Selection_SS::select_one(const Rarray<Solution*>& to_select_1,const Rarray<Solution*>& to_select_2,const Rarray<struct individual>& fitness_values,const unsigned int order,const bool replace) const
	{
		if(!prepareb)
			prepare2(to_select_1,to_select_2, fitness_values, replace);
		...
		{
			return fitness_values[_order[order]];
		}
	}
	
	void Selection_SS::setup(unsigned par1, unsigned par2)
	{
		_rs1 = par1;
		_rs2 = par2;
		
		_order = Rarray<int>(_rs1+_rs2);
	}

	int Selection_SS::prepare_one(const Rarray<Solution*>& to_select_1,const Rarray<Solution*>& to_select_2,const Rarray<struct individual>& fitness_values, unsigned s) const
	{
		const unsigned ps = to_select_1.size();
		double fit;
		Solution *s_o = (s>=ps)?to_select_2[s-ps]:to_select_1[s];

		if(fitness_values.size() <= ps) fit = s_o->fitness();
		else fit = fitness_values[s].fitness; 
	
		for(unsigned i = 0; i < _rs1; i++)
		{
//cout << endl << "\t Comparamos " << fitness_values[_order[i]].fitness << " " << fitness_values[s].fitness << endl;
			if(((fitness_values[_order[i]].fitness > fit) && (direction == minimize)) ||
				((fitness_values[_order[i]].fitness < fit) && (direction == maximize)))
				return i;
		}
		int pos = -1;
		double min_dif = s_o->distance(*to_select_1[0]);
		
		for(unsigned i = 0; i < _rs2; i++)
		{
			Solution *s_i = (i>=ps)?to_select_2[i-ps]:to_select_1[i];
			double dif = s_o->distance(*s_i);
			if(dif < min_dif)
			{
				pos = -1;
				min_dif = dif;
			...
			for(unsigned j = _rs1; j < _rs2; j++)
			{
				if(i == j) continue;
				Solution *s_j = (j>=ps)?to_select_2[j-ps]:to_select_1[j];
				dif = s_i->distance(*s_j);
				if(dif < min_dif)
				{
					pos = j;
					min_dif = dif;
	...
	void Selection_SS::prepare2(const Rarray<Solution*>& to_select_1,const Rarray<Solution*>& to_select_2,const Rarray<struct individual>& fitness_values,const bool replace) const
	{
		const int ps = to_select_1.size();
		
		for(unsigned i = 0; i < _rs1+_rs2; i++) _order[i] = i;
		
		if(replace)
		{
			_order[0] = prepare_one(to_select_1,to_select_2,fitness_values, ps);	
		}
		else
		{
			for(unsigned i = _rs1+_rs2; i < fitness_values.size(); ++i)
			{
//cout << endl << " Select: ";
//for(unsigned k = 0; k < _rs1+_rs2; k++) cout << fitness_values[_order[k]].fitness << " ";
//cout << endl << "Probando: " << i << " (" << fitness_values[i].fitness << ") " << endl;
				int pos = prepare_one(to_select_1,to_select_2,fitness_values,i);
				if(pos >= 0)
				{
//cout << "Es mejor que : " << pos << " (" << fitness_values[_order[pos]].fitness << ") " << endl;
					_order[pos] = i;
				}	
			}
		}
...
// Operator_Pool ------------------------------------------------------------------------
	...
	Operator_Pool::Operator_Pool(const Problem& pbm)
	{
		...
		_selectors.append(new Selection_SS(pbm.direction()));   	    	// 6
			...
			if (received_pid==-1)
			{
				...
				if(!final_phase && terminateQ(problem,*this,params))
				{
                    ...
                    acum_evaluations = acum_iterations* params.population_additional_size() +  
					                   nb_finalized_processes*(params.population_size1()+params.population_size2());
