...
skeleton newGASA
{
	...
	istream& operator>> (istream& is, Problem& pbm)
	{
		...
		sscanf(buffer," %d %d %lf %d",&pbm._nCustomers,&pbm._capacity,&pbm._maxRouteTime,&pbm._service_time);
...
// Intra_operator  --------------------------------------------------------------
	...
	Intra_Operator *Intra_Operator::create(const unsigned int _number_op)
	{
		switch (_number_op)
		{
			...
			case 3: return new Crossover_PMX2();break;
			case 4: return new Crossover_OX();break;
			case 5: return new Crossover_CX();break;
			case 6: return new Crossover_ER();break;
		}
	}
	...
	ostream& operator<< (ostream& os, const Intra_Operator& intra)
	{
		switch (intra.number_operator())
		{
			...
			case 3: os << (Crossover_PMX2&)intra;break;
			case 4: os << (Crossover_OX&)intra;break;
			case 5: os << (Crossover_CX&)intra;break;
			case 6: os << (Crossover_ER&)intra;break;
		}
		return os;
	}
...
//  Crossover_PMX1:Intra_operator -------------------------------------------------------------
	...
	void Crossover_PMX1::cross(Solution& sol1,Solution& sol2) const // dadas dos soluciones de la poblacion, las cruza
	{
		...
		// Copy old solutions
		Rarray<int> aux1(sol1.pbm().nCustomers());
		aux1=sol1.routes();
		Rarray<int> aux2(sol2.pbm().nCustomers());
		aux2=sol2.routes();

		int limit2=rand_int(1,sol1.pbm().nCustomers()-1);
	...
	Crossover_PMX2::Crossover_PMX2():Intra_Operator(3)
	{
		probability = new float[1];
	}
	...
	void Crossover_PMX2::cross(Solution& sol1,Solution& sol2) const // dadas dos soluciones de la poblacion, las cruza
	{
		...
		Rarray<int> new1(sol1.pbm().nCustomers());
		Rarray<int> new2(sol2.pbm().nCustomers());

		int limit2=rand_int(1,sol1.pbm().nCustomers()-1);
		int limit1=rand_int(0,limit2-1);

		for (i = 0; i < sol1.pbm().nCustomers(); i++)
		{
			new1[i] = -1;
			new2[i] = -1;
		}
	...
	Crossover_OX::Crossover_OX():Intra_Operator(4)
	{
		probability = new float[1];
	}
	...
	void Crossover_OX::cross(Solution& sol1,Solution& sol2) const // dadas dos soluciones de la poblacion, las cruza
	{
		int i;
		int j1,j2;
		const int max = sol1.pbm().nCustomers();
	...
	Crossover_CX::Crossover_CX():Intra_Operator(5)
	{
		probability = new float[1];
	}
	...
	void Crossover_CX::cross(Solution& sol1,Solution& sol2) const // dadas dos soluciones de la poblacion, las cruza
	{
		...
		for (int i = 0; i < sol1.pbm().nCustomers(); i++)
		{
			sol1.pos(i) = s1->pos(i);
			sol2.pos(i) = s2->pos(i);
		}
	...
	Crossover_ER::Crossover_ER():Intra_Operator(6)
	{
		probability = new float[1];
	}
	...
	{
		const int max = s1.pbm().nCustomers();
		int act,sig;

		sig = s1.pos(0);

		m[sig][0] = s1.pos(max-1);
		...
		sig = s2.pos(0);
		if(!is_in(s2.pos(max-1), m[sig], c_m[sig]))
		{
			m[sig][c_m[sig]] = s2.pos(max-1);
			c_m[sig]++;
		}
		...
		for(int i = 0; i < max-1;i++)
		{
			...
			if(!is_in(sig,m[act],c_m[act]))
			{
				m[act][c_m[act]] = sig;
				c_m[act]++;
			}

			if(!is_in(act,m[sig],c_m[sig]))
			{
				m[sig][c_m[sig]] = act;
				c_m[sig]++;
			}
		...
		if(!is_in(s1.pos(0),m[s1.pos(max-1)],c_m[s1.pos(max-1)]))
		{
			m[s1.pos(max-1)][c_m[s1.pos(max-1)]] = s1.pos(0);
			c_m[s1.pos(max-1)] ++;
		}
	}
	...
	{
		 _sc.set_contents_state_variable("_user_op_probability4",(char *)probability,1,sizeof(float));
	}

	void Crossover_ER::UpdateFromState(const StateCenter& _sc)
	{
		 unsigned long nbytes,length;
		 _sc.get_contents_state_variable("_user_op_probability4",(char *)probability,nbytes,length);
	}
