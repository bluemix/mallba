...
// Solver (superclass)---------------------------------------------------
	...
	void Solver_Lan::check_for_refresh_global_state() // Executed in process with pid 0
	{
  		...
  		while (!_end_trial)
		{
		 // checking for all processes
			...
			source_pid=(source_pid + 1) % nb_proc;
			if (source_pid==0) source_pid++;
			received_pid=0;
			received_pid=receive_local_state_from(source_pid);
			if (received_pid != 0)	// a packet has been received
			{
	...
	void Solver_Lan::run (const unsigned long int  max_evaluations)
	{
		...
		else
		{
			...
			for (int target_pid=1;target_pid<_netstream.pnumber()-1;target_pid++)
			{
				_netstream << set_target(target_pid);
			} 						
		}
	...
	void Solver_Lan::reset()
	{
		...
		if (mypid!=0)
		{
			...
			do
			{
				pending=false;
			 	_netstream._probe(any,pending);	
			 	if (pending) _netstream >> left_cost >> left_solution;				
			} while (pending);
		}
		_netstream << barrier; 
	}	
