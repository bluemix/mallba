...
//  Crossover:Intra_operator -------------------------------------------------------------
	...
	{
		int n = sol1.pbm().dimension();
		static Matrix<float> aleatoria1(n,n);
		static Matrix<float> aleatoria2(n,n);
		Matrix<float> aux = sol1.variables();
		...
		sol1.variables() = sol1.variables()* aleatoria1 + sol2.variables()*aleatoria2;
		sol2.variables() = aux * aleatoria2 + sol2.variables()*aleatoria1;

		sol1.parameters() = (sol1.parameters() + sol2.parameters()) * 0.5;
		sol2.parameters() = sol1.parameters();

		sol1.angles() = (sol1.angles() + sol2.angles()) ;

		for(int i = 0; i < ((n*(n - 1)) / 2); i++)
		{
			sol1.angle(i) = (float)fmod((double)sol1.angle(i),(double)4.0*PI) / 2.0;
			sol2.angle(i) = sol1.angle(i);
		}
	}
...
/*	void Crossover::cross(Solution& sol1,Solution& sol2) const // dadas dos soluciones de la poblacion, las cruza
	{
		...
		for(int i = 0; i < ((n*(n - 1)) / 2); i++)
			sol1.angle(i) = fmod(sol1.angle(i),4*PI) / 2.0;
	}

	void Crossover::execute(Rarray<Solution*>& sols) const
	{
		Solution sol = *sols[0];
		for(int i=0;i+1<sols.size();i=i+1)
			if (rand01()<=probability[0])
				cross(*sols[i],*sols[i+1]);
		if (rand01()<=probability[0])
			cross(*sols[sols.size()-1],sol);
	}

*/
	ostream& operator<< (ostream& os, const Crossover&  cross)
	{
		 os << "Crossover." << " Probability: " << cross.probability[0];
		 return os;
	}
...
//  Mutation: Sub_operator -------------------------------------------------------------
	...
	Mutation::Mutation():Intra_Operator(1),N(NULL)
	{
		probability = new float[4];
	}

	void Mutation::mutateAngles(Solution& sol) const
	{
		for(int i=0; i < ((sol.pbm().dimension() * (sol.pbm().dimension() - 1)) / 2);i++)
		{
			sol.angle(i) = (float) fmod(sol.angle(i) + probability[1]*(rand01()*2.0 - 1.0),2*PI);
		}
	}

	void Mutation::mutateParameters(Solution& sol) const
	{
		float par = (float) rand01()*2.0 - 1.0;

		for(int i=0;i< sol.pbm().dimension();i++)
		{
			float aux = exp(probability[2]*par + probability[3]*(rand01()*2.0 - 1.0));
			sol.parameter(i) = (float)( sol.parameter(i) * aux );
		}
	}

	void Mutation::mutateVariables(Solution& sol) const
	{
		int n = sol.pbm().dimension();
		int k = ((n*(n-1))/2) - 1;
		static Matrix<float> incr(n);

		for(int i = 0; i < n; i++)
			incr[i] = sol.parameter(i) * (*N)[i];

		for(int i = n-2; i >= 0; i--)
			for(int j = n-1; j > i; j--)
			{
				float angle = sol.angle(k);
				incr[i] = incr[i] * cos(angle) - incr[j] * sin(angle);
				incr[j] = incr[i] * sin(angle) + incr[j] * cos(angle);
				k--;
			}

		sol.variables() = sol.variables() + incr;
	}
	...
	void Mutation::execute(Rarray<Solution*>& sols) const
	{
		...
		{
			N = new Matrix<float>(sols[0]->pbm().dimension());
	...
	ostream& operator<< (ostream& os, const Mutation&  mutation)
	{
		os << "Mutation." << " Probability: " << mutation.probability[0]
				  << " Phi: " << mutation.probability[1]
				  << " Tau: " << mutation.probability[2]
				  << " Nu:  " << mutation.probability[3];
		return os;
	}
	...
	void Mutation::setup(char line[MAX_BUFFER])
	{
		int op;
		sscanf(line," %d %f",&op,&probability[0]);
		probability[1] = (5*PI/180.0);
		probability[2] = 1.0/sqrt(2*PI);
		probability[3] = 1.0/sqrt(sqrt(4*PI));
		assert(probability[0]>=0);
		assert(probability[1]>=0);
		assert(probability[2]>=0);
		assert(probability[3]>=0);
	}

	void Mutation::RefreshState(const StateCenter& _sc) const
	{
		_sc.set_contents_state_variable("_mutation_probability",(char *)probability,4,sizeof(probability));
	}
...
// SetUpParams -----------------------------------------------------------
	...
	: _independent_runs(0),
	  ...
	  _select_parents(0),
	  _select_offsprings(4),	// Selection of offspring : Select the best individuals. (fixed)
	...
	ostream& operator<< (ostream& os, const Statistics& stats)
	{
		...
		for (int i=0;i< stats.stats_data.size();i++)
			...
			os << endl
			   ...
			   << " Evaluations: " << stats.stats_data[i].nb_evaluation
...
// Solver (superclasse)---------------------------------------------------
	...
	const Population& Solver::population() const
	{
...
*/		cout << endl << "Current trial: " << current_trial();
		...
		cout << endl << "Evaluation best found in trial: " << evaluations_best_found_in_trial();
		...
		cout << endl << "Evaluation best found: " << evaluations_best_found();
