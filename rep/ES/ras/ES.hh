/*********************************************************************************************************
***												 												       ***
***				   					   Evolutionary Strategies Skeleton v1.5 					       ***
...
// Problem ----------------------------------------------------------------------------
  requires class Problem
  {
	public:
		...
		float maxvalue(const int index) const;

		float minvalue(const int index) const;

		int dimension() const;

	private:
		// Variables privadas dependientes del problema.
		int _dimension;
		float _minvalue;
		float _maxvalue;
...
//Solution ----------------------------------------------------------------------------
  requires class Solution
  {
	public:
		...
		float& variable(const int index);
		float& parameter(const int index);
		float& angle(const int index);
		Matrix<float>& variables();
		Matrix<float>& parameters();
		Matrix<float>& angles();

	private:
		Matrix<float> _variables;
		Matrix<float> _parameters;
		Matrix<float> _angles;
...
// Mutation ----------------------------------------------------------------------------------
  ...
  provides class Mutation: public Intra_Operator
  {
  	// Necesita 1 parámetros: 	0.- Probabilidad de Uso. (Este, el resto se dan por defecto)
	//							1.- phi : Factor de mutación de los ángulos (5*PI/180)
	//							2.- tau : Factor de mutación de los parámetros ((2*Pi)^-0.5)
	//							3.- nu  : Factor de mutación de los parámetros ((4*Pi)^-0.25)
	public:
		Mutation();
		virtual ~Mutation();
		...
		void mutateAngles(Solution &sol) const;
		void mutateParameters(Solution &sol) const;
		void mutateVariables(Solution &sol) const;
