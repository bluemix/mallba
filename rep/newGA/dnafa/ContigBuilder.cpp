...
ContigBuilder::ContigBuilder(int * order, int ** s, const newGA::Problem *pbm):_pbm(pbm)
{
	fragOrder = order;
	score = s;
}
...
void ContigBuilder::mergeContigs()
{
	...
	cerr << endl << "*** *** *** *** GA Contigs ( " << allContigs.size() << " ) *** *** *** ***" << endl << endl;
}
...
void ContigBuilder::buildConsensus()
{
	...
	vector<string> GACons;
	cerr << endl << "*** *** *** *** GA Contigs ( " << fragLayouts.size() << " ) *** *** *** ***" << endl << endl;
...
//		cout << endl << "Fragments Layout (" << j << ") =>" << endl << endl;
		...
		//	layoutBuilder.printAlignments();
			...
			GACons.push_back(layoutBuilder.getConsensus());
		}
		else
		{
			GACons.push_back(layout.at(0));
//			cout << layout.at(0) << endl;
		}
	}
	ConsensusBuilder * cb = new ConsensusBuilder(GACons);
	cb->buildConsensus();
