...
// Population ------------------------------------------------------
	...
	int NbContigs(const int *order,int **score,const int max,const int cutoff)
	{
		int cont = 1;
		for(int i=0; i < max-1; i++)
		{
			if(abs(score[order[i]][order[i+1]]) < cutoff) cont++;
		}
		return cont;
	}
	...
	void Population::Evaluate(Solution* sols,struct individual &_f)
	{
		if(_f.change)
		{
			_f.change  = false;
			int sign = -1;
			if(sols->pbm().direction() == minimize)
			{
				sign = 1;
			}
			_f.fitness1 = sols->fitness();
			_f.fitness = _f.fitness1 + sign * _average_cost2 * 0.010 * NbContigs(sols->fragments(), sols->pbm().detector->getScoreTable(), sols->pbm().numOfFragments,sols->pbm().cutoff);
			_evaluations++;
		}
	}
	...
	void Population::initialize()
	{
		...
		for (int i=0;i<_parents.size();i++)
		{
			...
			_fitness_values[i].index = i;
			_fitness_values[i].change = true;
			_average_cost2 += _parents[i]->fitness();
		}
		_average_cost2 /= _parents.size();
		evaluate_parents();
	}
	...
	void Population::evaluate_parents()
	{
		...
		for (int i=0;i<_fitness_values.size();i++)
		{
			...
			cost2 += _fitness_values[i].fitness1;
...
//		_average_cost2 =  cost2 / _fitness_values.size();
	}
...
//				cerr << "Entrado" << endl;
//				Solution s(best_solution_trial());
//			cerr << s;
